...
void ACATA::SetEnv(std::string ata_img_path, std::string ata_img_filename, std::string Media) {
	/* chimera: the media is mounted at the root of a FLAT file system under
	 * its own name, and that name is the whole path - there is no directory to
	 * join it to, and a "./" in front of it is a directory this file system
	 * does not have (patch 0019). An empty ata_img_path means "the name is the
	 * path", which is what a project always gives. */
	if (chimera_arcade_verbose)
		Console.WarningFmt("ata_img_path '{}' | ata_img_filename '{}'", ata_img_path, ata_img_filename);
    if (ata_img_path.empty() || Path::IsAbsolute(ata_img_filename))
