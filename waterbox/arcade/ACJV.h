...
/* InputBindingInfo and GenericInputBinding: the plain descriptors upstream
 * uses to name a control. The MAPPING layer they belong to is absent from this
 * core - a frontend has already done that work - but the JVS board's own
 * tables are written against these types, and they are what the package's
 * Arcade Panel declaration is generated from. */
#include "Config.h"
...
namespace ACJV {
    ...
    // chimera: the lightgun's aim is SetGunRelativeAim and nothing else - there
    // is no mouse in a sandbox and a movie cites axes, not a pointer.
