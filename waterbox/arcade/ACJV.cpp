...
#include "Host.h"	/* TRANSLATE_NOOP, for the names the DIP switches have */
#include <algorithm>
#include <array>
#include <map>
#include <cmath>
...
/* chimera: the Sinden border is a frame a desktop UI paints over the picture
 * so a camera gun can find the screen edge. A sandboxed core draws the
 * machine and nothing else, and the aim arrives as movie axes rather than
 * from a camera, so the border is off and stays off. */
static const bool s_sinden_border_enabled = false;
static const int s_sinden_border_mode = 0;
static const int s_sinden_border_thickness = 10;
static const bool s_lightgun_link_2p = false;
...
/* chimera: LoadConfig, CopyConfiguration and SetDefaultConfiguration lived
 * here, reading the DIP switches and every binding out of a desktop settings
 * file. A Chimera project states its own settings and the frontend owns the
 * bindings, so what remains of them is the DIP-switch setters below, which
 * cinterface.cpp calls with what the project pinned.
 */
...
void ACJV::SetDrumHit(u32 channel, bool pressed)
{
	if (channel < JVS_DRUM_CHANNEL_MAX)
		m_jvsDrumChannels[channel] = pressed ? 0xFFFF : 0; // max -> above IN/DAI; the big notes emerge from hitting both sides at once
}
...
void ACJV::SetGameId(const std::string& gameid)
{
	...
	for (u32 p = 0; p < JVS_PLAYER_COUNT; p++) // clear macro state; the frontend repushes masks on the input reload
	{
...
/* chimera: there is one aim source and it is the movie, so there is no
 * pointer device to pick between. */
...
static void UpdateLightgunFromMouse()
{
	...
	/* chimera: where the gun points is two axes in the movie, set through
	 * SetGunRelativeAim, and never a mouse read at the moment of the poll -
	 * that is the same rule the console's GunCon 2 follows (patch 0022). The
	 * board's own coordinate arithmetic below is untouched. */
	(void)unclamped;
	for (u32 p = 0; p < JVS_PLAYER_COUNT; p++)
	{
		const float dx = s_gunRelativeDX[p];
		const float dy = s_gunRelativeDY[p];
		if (camera)
		{
...
/* chimera: the touch panel's pointer is the movie's two axes as well, and the
 * software cursor a desktop paints for it is gone with the overlay. The stick
 * deflection the fork maps onto the screen is what a Chimera project sends
 * directly: -1 is the left or top edge, +1 the right or bottom one.
 */
...
static void UpdateTouchFromPointer()
{
	...
	const float cx = std::clamp((m_touchRel[0] + 1.0f) * 0.5f, 0.0f, 1.0f);
	const float cy = std::clamp((m_touchRel[2] + 1.0f) * 0.5f, 0.0f, 1.0f);
