...
#define ACATA_LOG(fmt, ...) if (chimera_arcade_verbose) Console.WriteLn(Color_Gray, "ACATA:" fmt __VA_OPT__(,) __VA_ARGS__)
...
namespace ACATA 
{
    ...
    /* chimera: the fork already calls these inline on the emulation thread -
     * the worker thread they were written for is gone from it - so the mutex
     * and the two condition variables it left behind go too. A sandbox has one
     * thread; a drive that answered on another would not be in the savestate. */
    namespace TH {
        extern bool isCHD;
        extern FILE* IMAGE;
