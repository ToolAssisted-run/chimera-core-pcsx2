 ...
 * video.rebuildOnStateLoad false. PCSX2 declares nothing, so it rebuilds. */
                                              ...
                                              uintptr_t c, uintptr_t d, uintptr_t e)
{
	...
	switch (op)
	{
		...
		case GL_OP_CONTEXT_ID:
			 ...
			 * (ChimeraCheckGLContext, waterbox/cinterface.cpp) and rebuilds
			 * them when it moves. Answering 0, which is what this host did
			 * before it had this case, means "cannot tell" - and a renderer
...
#include "gl-bridge-host.inc"
		...
		default:
			 ...
			 * for as long as the opcode existed. Measured 2026-09-21: 60
			 * frames of padtest.elf through run-wbx printed this line 60 times
			 * - ChimeraCheckGLContext asks once per advance - and nothing in
			 * run-gate.sh ever ran this dispatcher at all, so nothing was
			 * there to count them; the one leg that drives a GPU goes through
			 * chimera-run and the ENGINE's host half, not this one.
