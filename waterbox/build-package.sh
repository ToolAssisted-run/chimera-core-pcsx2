#!/bin/sh
# Builds the PCSX2 waterbox core package and installs it into a chimera
# checkout as build/Cores/pcsx2.chimeraCore.
#
...
pcsx2_pin="$(git -C "$root/extern/pcsx2" describe --tags --always 2>/dev/null || echo unknown)"
...
json.dump({
    ...
    "upstream": {"name": "PCSX2", "pin": "$pcsx2_pin"},
    "builtOn": "$os_id",
...
mkdir -p "$cores_dir"
zip_path="$cores_dir/pcsx2.chimeraCore"
rm -f "$zip_path"
...
for cache in "$chimera_root"/build/CoreCache/pcsx2-*; do
