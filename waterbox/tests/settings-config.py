...
if len(sys.argv) > 4:
    ...
    for decl_id, path in json.loads(sys.argv[4]).items():
        fw["PCSX2/" + decl_id] = path
# Pick this core explicitly: an install can hold several packages claiming the PS2 system, and --core only LOADS a package, it does not choose it.
cfg.setdefault("DefaultCores", {})["PS2"] = "PCSX2"
