#!/bin/sh
# Applies the numbered patches to the extern/pcsx2 submodule. Idempotent: a tree
...
root="$(cd "$here/.." && pwd)"
# PCSX2_TREE points it at another checkout: how this script is tested without
# touching the tree a build depends on
tree="${PCSX2_TREE:-$root/extern/pcsx2}"

if [ "$(git -C "$tree" rev-parse --show-toplevel 2>/dev/null)" != "$(cd "$tree" 2>/dev/null && pwd -P)" ]; then
	echo "extern/pcsx2 is not checked out; run:" >&2
	echo "  git -C $root submodule update --init --recursive extern/pcsx2" >&2
	exit 1
fi
...
if [ "$wrong" -ne 0 ]; then
	echo "extern/pcsx2 is partly patched. To start again from the submodule's HEAD:" >&2
	echo "  git -C extern/pcsx2 reset --hard && git -C extern/pcsx2 clean -fd && waterbox/apply-patches.sh" >&2
